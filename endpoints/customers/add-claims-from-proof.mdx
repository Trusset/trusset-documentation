...
<Warning>
  Verification runs against the root on chain, not against the root the manifest states. A bundle whose recomputed root does not match what the registry holds for this wallet is refused with `ROOT_MISMATCH`, whatever the manifest says. That is the point: the root is the anchor, and the manifest is a claim about it.
</Warning>
...
| `ROOT_MISMATCH` | `400` | The recomputed root does not match the identity's on-chain root |
