...
Returns the complete vault record with two enrichment blocks: `onChain`, the vault's live state read from the chain in one aggregate, and `holder`, the position of the wallet named in `address` (or `null` when no address is given).
...
<ResponseField name="data" type="object">
  <Expandable>
    <ResponseField name="onChain" type="object">
      <Expandable>
        <ResponseField name="readFailed" type="boolean">True when the chain could not be read. Every other field in this block is then absent — hold your last verified values rather than rendering zeros.</ResponseField>
        ...
        <ResponseField name="allocatedAssets" type="string">`totalAssets − idleAssets`: funds working in the operator's lending markets.</ResponseField>
        <ResponseField name="depositsPaused" type="boolean">True while the operator has paused deposits. Redemptions are never pausable.</ResponseField>
        <ResponseField name="performanceFeeBps" type="integer">Live fee. The contract refuses any increase.</ResponseField>
...
<Warning>
  `readFailed: true` means unavailable, never empty. A vault whose read failed still holds every deposit it held a minute ago. Surface stale-but-verified figures with a warning; never zeros.
</Warning>
...
if (data.onChain?.readFailed) {
  // hold last verified values — this is a loading problem, not a change to the vault
} else {
