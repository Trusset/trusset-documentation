...
A focused read for portfolio displays: the wallet's share balance, that balance at the current share price, and the share price itself. For the vault's full state alongside a position, use [Get Vault](/endpoints/vaults/get-vault) with `?address=`.
...
<ResponseField name="data" type="object">
  <Expandable>
    ...
    <ResponseField name="holder" type="object">`holderAddress`, `shares`, `sharesRaw`, `value`, `readFailed`. When `readFailed` is true, `shares` and `value` are `null` — unavailable, not zero.</ResponseField>
