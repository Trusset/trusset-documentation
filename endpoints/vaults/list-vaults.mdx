...
Returns every vault visible to your instance: your own vault (if your instance operates one) marked `own: true`, followed by every vault a bank issuer has published, across all instances on your network and environment. A vault that exists but was never published is visible only to the instance that operates it.

This list is a table read — it carries the vault records and their provenance, not live chain state. Share price, totals, and the deposit-pause flag live on [Get Vault](/endpoints/vaults/get-vault), which reads the chain once per vault you actually ask about.
...
<ResponseField name="data.vaults" type="array">
  <Expandable>
    ...
    <ResponseField name="operatorWallet" type="string">The operator's signing wallet — the vault's owner on-chain.</ResponseField>
    <ResponseField name="assetAddress" type="string">Settlement token address. Deposits, redemptions, and the share price are all denominated in it.</ResponseField>
    ...
    <ResponseField name="performanceFeeBps" type="integer">Operator's cut of the yield in basis points. It can only ever be lowered.</ResponseField>
    <ResponseField name="active" type="boolean">True once the operator published the vault to every issuer.</ResponseField>
    <ResponseField name="riskDisclosure" type="string">The operator's risk statement. Present it to investors before any deposit affordance.</ResponseField>
    <ResponseField name="riskDisclosureUrl" type="string">Link to the operator's full risk disclosure. `https` only.</ResponseField>
    <ResponseField name="operatedBy" type="object">`instanceName`, `issuerName`, `isBank`. Name the operator wherever you show the vault — it is their product, not Trusset's.</ResponseField>
  </Expandable>
</ResponseField>

<Note>
  Ordering interleaves your own vault(s) first, then published vaults newest-first. Pin on `own` client-side if you re-sort.
</Note>
