...
<Note>
  This endpoint always executes through the instance relayer, which must hold `OPERATOR_ROLE` on the router. There is no calldata mode. When `usdcProceeds` is greater than zero the relayer must be holding that amount, since settlement pulls it from the relayer wallet.
</Note>
...
| `SETTLE_PREFLIGHT_FAILED` | `400` | Simulation reverted, commonly an insufficient relayer balance |
