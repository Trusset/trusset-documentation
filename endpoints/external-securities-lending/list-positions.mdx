...
<Note>
  Positions are recorded when a loan is opened through this API in relayer mode, and are backfilled from chain events during reconciliation. A loan opened by a wallet signing calldata directly is picked up on the next reconciliation pass rather than at broadcast time.
</Note>
