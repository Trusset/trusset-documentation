...
<Warning>
  This call requires the instance relayer to hold `DEFAULT_ADMIN_ROLE` on the market contract, which is a higher privilege than the `LIQUIDATOR_ROLE` and router `OPERATOR_ROLE` the other liquidation endpoints need. Unlike those endpoints, a missing role is not detected in advance. It surfaces as a generic `TIMEOUT_PREFLIGHT_FAILED` with no remediation calldata. Confirm the relayer holds market admin before relying on this path.
</Warning>
...
| `TIMEOUT_PREFLIGHT_FAILED` | `400` | Simulation reverted. The 7 days have not elapsed, or the relayer lacks market `DEFAULT_ADMIN_ROLE` |
