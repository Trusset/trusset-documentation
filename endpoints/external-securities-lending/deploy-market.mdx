...
<ParamField body="liquidationOperator" type="string" required>
  Wallet receiving `OPERATOR_ROLE` on the market's liquidation router. This is the address permitted to withdraw seized collateral for sale and settle proceeds. Use your instance relayer address to drive settlement through this API. Must not be the zero address.
</ParamField>
...
In relayer mode the endpoint returns `201` with the recorded market and its readiness state.
...
<ResponseField name="data" type="object">
  <Expandable>
    ...
    <ResponseField name="relayerAddress" type="string">Instance relayer address, or null when none is configured.</ResponseField>
    ...
    <ResponseField name="oracleSignerAuthorized" type="boolean">Whether the relayer can sign prices for this market's oracle.</ResponseField>
    <ResponseField name="liquidatorRoleGranted" type="boolean">Whether the relayer holds `LIQUIDATOR_ROLE` on the market.</ResponseField>
...
const res = await fetch(
  'https://api.trusset.org/lending-external-securities/api/markets/deploy',
  {
    ...
    body: JSON.stringify({
      ...
      initialPrice: '104.82',
      collateralAgent: '0x555...666',
      liquidationOperator: relayerAddress,
      mode: 'CUSTODY',
...
| `DEPLOYER_NOT_AUTHORIZED` | `403` | The instance relayer is not on the factory's authorized deployer allowlist |
| `DEPLOYER_AUTHORIZATION_FAILED` | `502` | Adding the relayer to the allowlist failed on-chain |
...
Relayer mode can additionally return any [relayer error](/endpoints/introduction#relayer-errors).
