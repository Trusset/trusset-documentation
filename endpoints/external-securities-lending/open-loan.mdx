...
In calldata mode the response is always `SIGN_TRANSACTIONS` with a `mode` field, and the step count depends on the market.
...
In relayer mode the approval is handled automatically for custody markets.

## Response Fields

Relayer mode returns `201`.
...
<Note>
  Minimum and maximum borrow are pre-checked in calldata mode only. In relayer mode an out-of-range amount reverts on-chain rather than returning `BELOW_MIN_BORROW` or `EXCEEDS_MAX_BORROW`. The maximum borrow check is also skipped whenever a `signedPrice` is supplied, since the contract will price against the signature rather than the stored value.
</Note>
...
| `BELOW_MIN_BORROW` | `400` | `borrowAmount` is under the market's `minBorrowAmount`. Calldata mode only |
| `EXCEEDS_MAX_BORROW` | `400` | `borrowAmount` exceeds what the collateral supports. Calldata mode only |
