...
The call executes through the instance relayer, which must hold `DEFAULT_ADMIN_ROLE` on the market. If it does not, the response still succeeds but returns `requiresUserAction` with calldata for the market admin's wallet to execute instead.

<Note>
  This endpoint has no `useRelayer` parameter. It always attempts relayer execution and falls back to returning calldata.
</Note>
...
<ResponseField name="data" type="object">
  <Expandable>
    ...
    <ResponseField name="requiresUserAction" type="boolean">Present and `true` when the relayer could not execute. Pair with `configurationCalldata`.</ResponseField>
...
<Warning>
  Revoking the role from the instance relayer disables liquidation through this API for that market. Loans will still become liquidatable, but nothing will act on them until the role is restored or another liquidator is authorized.
</Warning>
...
const res = await fetch(
  `https://api.trusset.org/lending-external-securities/api/markets/${marketId}/liquidator-role`,
  {
    ...
    body: JSON.stringify({ address: relayerAddress, action: 'grant' })
  }
);
  ...
  "data": {
    ...
    "configurationCalldata": {
      "liquidatorRole": {
        ...
        "data": "0x...",
        "description": "Grant LIQUIDATOR_ROLE to 0x123...456"
      }
    }
  }
}
...
```json Error - Invalid Address
{
  "success": false,
  "error": {
    "code": "LIQUIDATOR_ROLE_UPDATE_FAILED",
