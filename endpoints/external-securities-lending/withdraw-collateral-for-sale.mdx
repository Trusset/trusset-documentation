...
<ParamField path="liquidationId" type="string" required>
  Liquidation ID on the market's router, from [Get Pending Liquidations](/endpoints/external-securities-lending/get-pending-liquidations) or from the [Liquidate Loan](/endpoints/external-securities-lending/liquidate) response.
</ParamField>
...
| `RECIPIENT_REQUIRED` | `400` | The market predates the fixed sale recipient, so `recipientAddress` must be supplied |
