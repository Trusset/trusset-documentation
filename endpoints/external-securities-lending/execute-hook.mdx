...
<Warning>
  A hook with `actionType` set to `buy` submits a real bid through the issuer relayer when the conditions pass. Test with `inform` first, or set `maxBuyPrice` to a value the live auction cannot reach.
</Warning>
...
Skipped `buy` actions return `result: false` with a `reason` field, for example `Price exceeds max buy price` or `Auction not active`. An insufficient relayer balance returns `success: false` with an `error` describing the shortfall in the market's borrow asset.
