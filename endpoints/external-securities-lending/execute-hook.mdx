...
<Warning>
  A hook with `actionType` set to `buy` never submits a bid. When the conditions pass it returns `requiresUserAction: true` with the unsigned `approve` and `bidOnAuction` steps for you to sign, and records the execution as unfulfilled with `reason: "requires_wallet_signature"`.
</Warning>
