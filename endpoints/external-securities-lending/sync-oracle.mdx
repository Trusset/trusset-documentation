...
<Warning>
  The wallet that signs must be an authorized signer on the oracle, which the oracle owner grants. The API checks your instance's registered wallet before building the calldata and refuses with `SIGNER_NOT_AUTHORIZED` if it is not authorized. Check `signerAuthorized` on [Get Oracle Status](/endpoints/external-securities-lending/get-oracle-status) first.
</Warning>
