...
Writes a new price to the market's oracle through the instance relayer. This is how external securities markets are priced: the issuer publishes a net asset value and pushes it here.
...
<Warning>
  This submits a blockchain transaction. The instance relayer must be an authorized signer on the oracle, which the oracle owner grants. Check `signerAuthorized` on [Get Oracle Status](/endpoints/external-securities-lending/get-oracle-status) first.
</Warning>
...
| `SIGNER_NOT_AUTHORIZED` | `403` | The instance relayer is not authorized on the oracle |
...
| `RELAYER_FROZEN` | `400` | The relayer is frozen |
| `RELAYER_LIMIT_EXCEEDED` | `400` | The relayer's daily transaction limit is exhausted. Resets at the next UTC day boundary |
