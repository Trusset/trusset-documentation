...
<ResponseField name="data" type="object">
  <Expandable>
    ...
    <ResponseField name="amount" type="string">Collateral released, echoed back. Relayer mode only.</ResponseField>
    <ResponseField name="action" type="string">`SIGN_TRANSACTION` in calldata mode.</ResponseField>
    <ResponseField name="transaction" type="object">`{ to, data }` for the `withdrawCollateral` call. Calldata mode only.</ResponseField>
  </Expandable>
</ResponseField>
...
curl -X POST "https://api.trusset.org/lending-external-securities/api/positions/{marketId}/withdraw-collateral" \
  ...
  -d '{"loanId": 5, "amount": "100", "useRelayer": true}'
```
</RequestExample>
