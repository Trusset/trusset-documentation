...
<ParamField body="signedPrice" type="object">
  EIP-712 signed price from [Sign Price](/endpoints/external-securities-lending/sign-price), applied atomically with the liquidation. Omit to price against the stored oracle value. Nothing is signed on your behalf. A stale stored price is refused with `PRICE_STALE`. Either push a NAV with [Sync Oracle Price](/endpoints/external-securities-lending/sync-oracle), or supply a `signedPrice` here.
  <Expandable>
  ...
  <Expandable>
    ...
    <ResponseField name="readiness" type="object">
      ...
      <Expandable>
        ...
        <ResponseField name="routerTokenEligible" type="boolean">Whether the collateral token permits the router to hold it.</ResponseField>
        <ResponseField name="liquidatorRoleReady" type="boolean">Whether the resolved signer holds `LIQUIDATOR_ROLE`. Null when it could not be read.</ResponseField>
        <ResponseField name="operatorRoleReady" type="boolean">Whether the resolved signer holds the router's operator role.</ResponseField>
        <ResponseField name="signerAddress" type="string">The address readiness was resolved against.</ResponseField>
        <ResponseField name="collateralTransferAllowed" type="boolean">Whether the token would allow the seizure transfer to the router. `collateralTransferChecked` and `collateralTransferCode` accompany it. See [Transfer Restriction Codes](/endpoints/external-securities-lending/get-liquidation-config#transfer-restriction-codes).</ResponseField>
        <ResponseField name="saleRecipient" type="string">Recipient the lender of record fixed for onward sale, or null.</ResponseField>
        <ResponseField name="saleDeliveryAllowed" type="boolean">Whether the token would allow the onward transfer to that recipient. `saleDeliveryChecked` and `saleDeliveryCode` accompany it.</ResponseField>
...
| `LOAN_NOT_LIQUIDATABLE` | `400` | The loan's health factor is at or above `1.0`. The error carries `loan` |
...
<Tip>
  A missing `LIQUIDATOR_ROLE` shows as `liquidatorRoleReady: false` in `readiness` rather than as an error. Resolve it through [Set Liquidator Role](/endpoints/external-securities-lending/set-liquidator-role) when the market admin drives the grant.
</Tip>
