...
Returns the market's recorded activity with pagination counters. Records are written by the backend when an action is executed through the relayer or observed during a chain sync. Actions signed and broadcast directly by a client wallet in calldata mode are not recorded here, since the backend never sees them.
