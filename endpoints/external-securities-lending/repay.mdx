...
Two transactions are required: an approval on the borrow asset, then the repayment. In relayer mode the approval is handled automatically.
...
<ResponseField name="data" type="object">
  <Expandable>
    ...
    <ResponseField name="repaidAmount" type="string">Amount submitted, echoed back. Relayer mode only.</ResponseField>
    <ResponseField name="action" type="string">`SIGN_TRANSACTIONS` in calldata mode.</ResponseField>
    <ResponseField name="steps" type="array">Ordered `approve` then `repay` transactions. Calldata mode only.</ResponseField>
  </Expandable>
</ResponseField>
...
await fetch(
  `https://api.trusset.org/lending-external-securities/api/positions/${marketId}/repay`,
  {
    ...
    body: JSON.stringify({ loanId: 5, amount: payoff.toFixed(6), useRelayer: true })
  }
);
```
</RequestExample>
