...
<ResponseField name="data" type="object">
  <Expandable>
    ...
    <ResponseField name="collateralPurchased" type="string">Collateral bought, echoed back. Relayer mode only.</ResponseField>
    <ResponseField name="usdcPaid" type="string">Amount paid, in borrow asset units, computed at the price at execution time. Relayer mode only.</ResponseField>
    <ResponseField name="action" type="string">`SIGN_TRANSACTION` in calldata mode.</ResponseField>
    <ResponseField name="transaction" type="object">`{ to, data }` for the `bidOnAuction` call. Calldata mode only.</ResponseField>
  </Expandable>
</ResponseField>

<Warning>
  Calldata mode returns the bid transaction alone, with no approval step. The bidder must already have approved the market contract to spend the borrow asset, otherwise the bid reverts. Relayer mode handles the approval automatically.
</Warning>
...
curl -X POST "https://api.trusset.org/lending-external-securities/api/liquidations/{marketId}/bid-on-auction" \
  ...
  -d '{"auctionId": 3, "collateralAmount": "100", "useRelayer": true}'
```
