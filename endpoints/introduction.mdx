...
<CardGroup cols={2}>
  ...
  <Card title="Stock Trading" icon="chart-line" href="/endpoints/stock-trading/introduction">
    Order book trading for tokenized stocks. Manage books and their custody setup, run continuous or periodic-auction matching, enforce price references, request quotes (RFQ), and place orders on the off-chain settlement surface.
  </Card>
  <Card title="External Securities Trading" icon="building-columns" href="/endpoints/external-securities-trading/introduction">
    Order book trading for imported ERC-3643 security tokens, settled on chain against both traders' EIP-712 signatures. Prepare and submit signed orders, read live compliance and balances, and drive the operator's settlement queue.
  </Card>
  <Card title="Commodity Trading" icon="coins" href="/endpoints/commodity-trading/introduction">
    Order book trading for tokenized commodities. Manage books and custody setup, pool depth across attested fungibility classes, enforce price references, and read depth, trades and daily history.
  </Card>
