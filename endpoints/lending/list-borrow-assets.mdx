...
Returns the settlement assets Trusset curates for your instance type, plus the decimal bounds enforced on custom assets. Use this to populate a borrow asset selector before calling [Deploy Market](/endpoints/lending/deploy-market).
...
When you pass a `borrowAssetAddress` that fails validation, [Deploy Market](/endpoints/lending/deploy-market) returns one of:
