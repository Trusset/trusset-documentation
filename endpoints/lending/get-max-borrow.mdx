...
Returns the largest amount the market will lend against a given quantity of collateral, computed by the contract at the current oracle price and collateral factor. Use it to drive a borrow slider before calling [Open Loan](/endpoints/lending/open-loan).
...
<Warning>
  When `isStale` is `true`, opening a loan against the stored price will revert. Either push a fresh NAV with [Sync Oracle Price](/endpoints/lending/sync-oracle) or attach a `signedPrice` from [Sign Price](/endpoints/lending/sign-price) to the borrow call.
</Warning>
