...
<ResponseField name="data" type="object">
  <Expandable>
    <ResponseField name="complete" type="boolean">True when the market has a lender of record and every step is either done or delegated.</ResponseField>
    <ResponseField name="mode" type="string">`FREEZE` or `CUSTODY`. Determines which steps appear.</ResponseField>
    <ResponseField name="priceSource" type="string">`NAV`, `MARKET` or `ORACLE`, read from the contract where possible and from the record otherwise.</ResponseField>
    <ResponseField name="collateralAgent" type="string">Named pledgee recorded at deployment.</ResponseField>
    <ResponseField name="curator" type="string">The institution holding the lender-of-record role, or `null`.</ResponseField>
    <ResponseField name="seekingCurator" type="boolean">`true` while the market has no lender of record.</ResponseField>
    <ResponseField name="realizationMode" type="string">How that institution declared it will realize seized collateral: `EXCHANGE_SALE`, `ISSUER_REDEMPTION` or `AUCTION`.</ResponseField>
    <ResponseField name="saleRecipient" type="string">Where seized collateral is sent for realization.</ResponseField>
    <ResponseField name="operable" type="boolean">Whether the market has been adopted and can be operated at all. `complete` can never be true while this is `false`.</ResponseField>
    <ResponseField name="deploymentReadable" type="boolean">Whether the factory record for this market could be read. When `false`, the lender-of-record fields above fall back to the stored record.</ResponseField>
    <ResponseField name="governance" type="object">`marketAdminHeldByInstance`, `oracleOwnedByInstance` and `oracleOwner`. Says which of the two administrative keys your instance actually holds, which decides which steps you can sign yourself.</ResponseField>
    ...
    <ResponseField name="steps" type="array">
      <Expandable>
        ...
        <ResponseField name="delegated" type="boolean">`true` when the step is not this instance's to perform and another party already covers it. A delegated step counts toward `complete` without being `done`.</ResponseField>
        <ResponseField name="blocked" type="boolean">`true` when the step cannot be achieved as things stand, rather than merely being outstanding.</ResponseField>
        <ResponseField name="blockedReason" type="string">Why, in words. Present alongside `blocked`.</ResponseField>
        <ResponseField name="blockedCode" type="string">The machine-readable form of the same. Present alongside `blocked`.</ResponseField>
      </Expandable>
    </ResponseField>
...
<Tip>
  `grantLiquidatorRole` can be executed directly through [Set Liquidator Role](/endpoints/lending/set-liquidator-role) when the registered wallet holds the market admin role.
</Tip>
