...
Two things a market operator often looks for are not in this struct. `maxPriceAge`, the staleness window, belongs to the oracle and is on [Get Oracle Status](/endpoints/lending/get-oracle-status). The rate mode, the term offering and the reserve ratio belong to the interest model and the market's own storage, and are on [Get Market Metrics](/endpoints/lending/get-metrics).
