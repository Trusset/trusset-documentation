...
<ParamField body="actionType" type="string">
  Required for `afterLiquidation`. `buy` prices a bid on the resulting Dutch auction and returns the unsigned steps to place it, which you sign yourself; nothing is bid automatically. `inform` posts to `webhookUrl`.
</ParamField>
