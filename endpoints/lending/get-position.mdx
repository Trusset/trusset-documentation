...
<ResponseField name="data" type="object">
  <Expandable>
    ...
    <ResponseField name="compliance" type="object">What the collateral token would currently permit for this borrower: whether the address is frozen, whether the identity is still verified, whether the token is paused, and whether a release would be delivered or escrowed. Identical in shape to the block on [Get Loan](/endpoints/lending/get-loan#compliance), where each field is described. `null` when the probe could not run.</ResponseField>
    <ResponseField name="transactions" type="array">Every transaction on this position, newest first.</ResponseField>
  </Expandable>
</ResponseField>

<Note>
  `status` is read from the stored record and is not re-derived from chain state here. A loan repaid without a confirm call can still read `ACTIVE` until the next reconciliation pass. For a live view of whether the loan is open, read `active` from [Get Loan](/endpoints/lending/get-loan), or call [List Positions](/endpoints/lending/list-positions), which reconciles before returning.
</Note>
