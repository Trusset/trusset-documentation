...
<ResponseField name="data" type="object">
  <Expandable>
    ...
    <ResponseField name="maxPriceAge" type="integer">Staleness threshold in seconds, read from the oracle. It is an oracle setting and is not part of the market config struct. Conditional on `configured`.</ResponseField>
    ...
    <ResponseField name="ownerIsInstanceWallet" type="boolean">Whether that owner is a verified wallet of your instance. When `false`, price authority sits with another party and no wallet of yours can authorize signers on this oracle. Conditional on `configured`.</ResponseField>
    ...
    <ResponseField name="priceSigner" type="string">The wallet the lender of record declared as this market's price authority when it took the role, or `null`. It is not necessarily your registered wallet. It is the address [Sync Oracle Price](/endpoints/lending/sync-oracle) and [Sign Price](/endpoints/lending/sign-price) resolve to when you name none.</ResponseField>
    <ResponseField name="priceSignerAuthorized" type="boolean">Whether that declared signer is authorized on the oracle. `null` when no signer is declared.</ResponseField>
    <ResponseField name="error" type="string">Present only when `configured` is `false`.</ResponseField>
  </Expandable>
</ResponseField>

<Warning>
  The failure response is a different shape, not an error status. When `configured` is `false` the endpoint still returns `200` with `price: "0"`, `isStale: true`, and no `priceAge`, `maxPriceAge`, `maxDeviationBps`, `owner`, `ownerIsInstanceWallet` or `signerAddress`. Guard on `configured` before reading those fields.
</Warning>

## Price Decimals

Oracle prices carry the market's borrow asset decimals, not a fixed precision. A USDC market prices at 6 decimals, a DAI market at 18. Read `borrowAssetDecimals` from [Get Market](/endpoints/lending/get-market) rather than assuming.
...
```json Response
{
  ...
  "data": {
    ...
    "configured": true,
    "owner": "0x333...444",
    "ownerIsInstanceWallet": false,
    "signerAddress": "0x123...456",
    "signerAuthorized": false,
    "priceSigner": "0x9c1e...4b70",
    "priceSignerAuthorized": true
  }
}
```
